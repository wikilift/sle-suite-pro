...
def build_authenticate(psc):
    ...
    return [0xFF, ACS.VERIFY_PSC, 0x00, 0x00, 0x03] + psc
...
def build_3w_write(addr, value, protect=True):
    cmd = SLE3W.WRITE_AND_ERASE_WITH_PROTECT if protect else SLE3W.WRITE_AND_ERASE_NO_PROTECT
...
def build_hid_read_protection(addr: int, length: int):
    return [0xFF, HID.READ_PROTECTION, (addr >> 8) & 0xFF, addr & 0xFF, length]

def build_hid_compare_and_protect(addr: int, value: int):
    return [0xFF, HID.COMPARE_AND_PROTECT, (addr >> 8) & 0xFF, addr & 0xFF, 0x01, value]
