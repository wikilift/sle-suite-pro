...
class BaseCard:
    ...
    def authenticate(self, psc: list[int]):
        ...
        if stored_psc == list(psc):
            ...
            self._log(f"{tr('log.auth_ok')}. {tr('log.security_counter')}={counter_after}")
        else:
            self.is_authenticated = False
            self._log(f"{tr('log.auth_fail')}. {tr('log.security_counter')}={counter_after}")
            raise Exception(f"{tr('log.auth_fail')}. Quedan {counter_after} {tr('log.security_counter')}.")
